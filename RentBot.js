...
const rentfromxeon = async (Zex, m, from) => {
const { sendImage, sendMessage } = Zex;
...
let qutsnya = await Zex.sendContact(callerId, owner)
await Zex.sendMessage(callerId, { text: `Block Automatic System!!!\nDon't Call Bots!!!\nPlease contact the owner to open the block!!!`}, { quoted : qutsnya })
await sleep(8000)
await Zex.updateBlockStatus(callerId, "block")
}
})
...
Zex.ev.on("connection.update", async up => {
...
if (connection == "open") {
Zex.id = Zex.decodeJid(Zex.user.id)
Zex.time = Date.now()
global.conns.push(Zex)
await m.reply(`*Connected to ${botname}*\n\n*User :*\n _*× id : ${Zex.decodeJid(Zex.user.id)}*_`)
user = `${Zex.decodeJid(Zex.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); Zex.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Zex.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); Zex.logout(); }
