...
try {
    	...
    	const banUser = await Zex.fetchBlocklist()
...
var xeonytimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6283890422366") {
			let evaled,
				text = q,
			...
			try {
				...
				await Zex.sendMessage(from, { text: evaled }, { quoted: m });
			} catch (e) {
				Zex.sendMessage(from, { text: String(e) }, { quoted: m });
			}
		}
try {
...
Zex.sendPresenceUpdate('available', from)
...
if (m.sender.startsWith('92')) return Zex.updateBlockStatus(m.sender, 'block')

async function sendZexMessage(chatId, message, options = {}){
    let generate = await generateWAMessage(chatId, message, options)
    let type2 = getContentType(generate.message)
    if ('contextInfo' in options) generate.message[type2].contextInfo = options?.contextInfo
    if ('contextInfo' in message) generate.message[type2].contextInfo = message?.contextInfo
    return await Zex.relayMessage(chatId, generate.message, { messageId: generate.key.id })
}
...
const reply = (teks) => {
Zex.sendMessage(from, { text: teks ,
contextInfo:{
forwardingScore: 9999999, 
isForwarded: true
}
...
Zex.sendImageAsSticker(m.chat, pesan, m, { packname: global.packname, author: global.author })
}

try {
ppuser = await Zex.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
const sendvn = (teks) => {
Zex.sendMessage(from, { audio: teks, mimetype: 'audio/mp4', ptt: true }, { quoted: m })
}
...
Zex.sendMessage(m.chat, { audio: audiobuffy, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
}
}
for (let BhosdikaXeon of StickerXeon){
if (budy === BhosdikaXeon){
let stickerbuffy = fs.readFileSync(`./media/sticker/${BhosdikaXeon}.webp`)
Zex.sendMessage(m.chat, { sticker: stickerbuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of ImageXeon){
if (budy === BhosdikaXeon){
let imagebuffy = fs.readFileSync(`./media/image/${BhosdikaXeon}.jpg`)
Zex.sendMessage(m.chat, { image: imagebuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of VideoXeon){
if (budy === BhosdikaXeon){
let videobuffy = fs.readFileSync(`./media/video/${BhosdikaXeon}.mp4`)
Zex.sendMessage(m.chat, { video: videobuffy }, { quoted: m })
}
}

if (m.isGroup && m.mtype == 'viewOnceMessage') {
let teks = `╭「 *Anti ViewOnce* 」\n├ *Name* : ${pushname}\n├ *User* : @${m.sender.split("@")[0]}\n├ *Clock* : ${time2}\n└ *Message* : ${m.mtype}`
Zex.sendMessage(m.chat, { text: teks, mentions: [m.sender] }, { quoted: m })
await sleep(500)
...
let gdyr = await Zex.sendMessage(from, {image: { url: Lehd.thumb } , caption: `Channel Name : ${ghed.player_response.videoDetails.author}
...
.on('finish', async () => {
await Zex.sendMessage(from, { video: fs.readFileSync(mp4File), caption: mess.succes, gifPlayback: false }, { quoted: gdyr })
...
let gedeyeer = await Zex.sendMessage(from, { image: { url: Puxa.thumb } , caption: `Channel Name : ${PlXz.player_response.videoDetails.author}
...
.on('finish', async () => {
await Zex.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: gedeyeer })
...
async function sendFile(jid, media, options={}) {
        ...
        return Zex.sendMessage(jid, { [type]: file.data, ...options }, { ...options })
      }
    ...
    m.reply(`This feature is specifically for premium user, contact the owner to become premium user`)
        ...
        if (isAutoSticker) {
            if (/image/.test(mime) && !/webp/.test(mime)) {
                ...
                await Zex.sendImageAsSticker(from, mediac, m, { packname: global.packname, author: global.author })
                console.log(`Auto sticker detected`)
            } else if (/video/.test(mime)) {
                if ((quoted.msg || quoted).seconds > 11) return
                let mediac = await quoted.download()
                await Zex.sendVideoAsSticker(from, mediac, m, { packname: global.packname, author: global.author })
            }
        }
...
// Anti Link
        ...
        let gclink = (`https://chat.whatsapp.com/`+await Zex.groupInviteCode(m.chat))
        let isLinkThisGc = new RegExp(gclink, 'i')
        let isgclink = isLinkThisGc.test(m.text)
        if (isgclink) return Zex.sendMessage(m.chat, {text: `\`\`\`「 Group Link Detected 」\`\`\`\n\nYou won't be kicked by a bot because what you send is a link to this group`})
        if (isAdmins) return Zex.sendMessage(m.chat, {text: `\`\`\`「 Group Link Detected 」\`\`\`\n\nAdmin has sent a link, admin is free to post any link`})
        if (XeonTheCreator) return Zex.sendMessage(m.chat, {text: `\`\`\`「 Group Link Detected 」\`\`\`\n\nOwner has sent a link, owner is free to post any link`})
        kice = m.sender
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
			Zex.sendMessage(from, {text:`\`\`\`「 Group Link Detected 」\`\`\`\n\n@${kice.split("@")[0]} Has been kicked because of sending group link in this group`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
            }            
        }
...
kice = m.sender
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Wa.me Link Detected 」\`\`\`\n\n@${kice.split("@")[0]} Has been kicked because of sending wa.me link in this group`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
} else {
}
  if (antiWame)
...
//antivirtex by xeon
  ...
  if (!isBotAdmins) return m.reply(mess.botAdmin)
          await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
			Zex.sendMessage(from, {text:`\`\`\`「 Virus Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending virus in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
  }
  }
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			await Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Bad Word Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} was kicked because of using bad words in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})}
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 YouTube Video Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending youtube video link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 YouTube Channel Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending youtube channel link in this group`, contextInfo:{mentionedJid:[m.sendet]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Instagram Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending instagram link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Facebook Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending facebook link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Telegram Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending telegram link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Tiktok Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending tiktok link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (XeonTheCreator) return m.reply(bvl)
        await Zex.sendMessage(m.chat,
			...
			Zex.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Zex.sendMessage(from, {text:`\`\`\`「 Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
Zex.sendMessage(m.chat, { text: sendResponList(m.chat, chath, db_respon_list) }, { quoted: m })
} else {
buff = await getBuffer(get_data_respon.image_url)
Zex.sendImage(m.chat, buff, `${get_data_respon.response}`, m)
}
}
...
if (!isCmd && isAlreadyZexList(chath, dblist)) {
var getraindata = getDataZexList(chath, dblist)
if (getraindata.isImage === false) {
Zex.sendMessage(m.chat, { text: sendZexList(chath, dblist) }, { quoted: m })
} else {
buff = await getBuffer(getraindata.image_url)
Zex.sendImage(m.chat, buff, `${getraindata.response}`, m)
}
}
...
emoji.get(satu)
.then(emoji => {
Zex.sendMessage(from, { caption: mess.success, image: {url: emoji.images[dua].url} }, {quoted:m})
})
} catch (e) {
...
case 'rentbotlist': 
try {
let user = [... new Set([...global.conns.filter(Zex => Zex.user).map(Zex => Zex.user)])]
te = "*Rentbot List*\n\n"
for (let i of user){
y = await Zex.decodeJid(i.id)
te += " × User : @" + y.split("@")[0] + "\n"
te += " × Name : " + i.name + "\n\n"
}
Zex.sendMessage(from,{text:te,mentions: [y], },{quoted:m})
} catch (err) {
...
case 'alive': case 'panel': case 'list': case 'menu': case 'help': case '?': {
            ...
            xeonezy = `*🚩 Hi ${pushname} ${xeonytimewisher}, I Am ${global.botname}, Please Use Wisely And Don't Spam.*

❐ *Bot Infromation :*    
  ∘ 𝗦𝗽𝗲𝗲𝗱 : ${latensie.toFixed(4)} miliseconds
  ∘ 𝗥𝘂𝗻𝘁𝗶𝗺𝗲 : ${runtime(process.uptime())}
  ∘ 𝗢𝘄𝗻𝗲𝗿 : @${ownernya.split('@')[0]}
  ∘ 𝗠𝗼𝗱𝗲 : ${Zex.public ? 'Public' : `Self`}
...
sendZexMessage(from, { 
text: `🚩 Hi @${sender.split("@")[0]}\n\n${allmenu(prefix, hituet)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${ownermenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${othermenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${downloadmenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${groupmenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${funmenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${stalkermenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${randphotomenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${randvideomenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${textpromenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${photooxymenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${ephoto360menu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${nsfwmenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
sendZexMessage(from, { 
text: `Hi @${sender.split("@")[0]}\n\n${animemenu(prefix)}, This Is A List Of All Menus In The Bot`,
mentions:[sender],
contextInfo:{
mentionedJid:[sender],
"externalAdReply": {
...
Zex.sendImage(m.chat, resd.data.data[0].url, `*Result From Ai-Image*`, m)
break
case 'sound1':
case 'sound2':
case 'sound3':
...
case 'sound160':
case 'sound161':
Zex_dev = await getBuffer(`https://github.com/DGXeon/Tiktokmusic-API/raw/master/tiktokmusic/${command}.mp3`)
await Zex.sendMessage(m.chat, { audio: Zex_dev, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
break
case 'friend':
case 'searchfriend':{
if (!isPrem) return replyprem(mess.premium)
...
setTimeout(() => {
Zex.sendMessage(from, {text: `Here @${teman.split("@")[0]}`, mentions: [teman]}, { quoted : m })
}, 9000)
}
break
...
let xeonquotx= await Zex.serializeM(await m.getQuotedObj())
...
break
         case 'tts': case 'say':{
             ...
             Zex.sendMessage(m.chat, { audio: { url: tts.result }, mimetype: 'audio/mp4', ptt: true, fileName: `${text}.mp3` }, { quoted: m })
         	}
         break
case 'igstalk':{
if (!isPrem) return replyprem(mess.premium)
...
replygcxeon(mess.wait)
aj = await igstalk(`${q}`)
Zex.sendMessage(m.chat, { image: { url : aj.profile }, caption: 
`*/ Instagram Stalker \\*
...
Zex.sendMessage(m.chat, { image: { url : aj.profile_pic }, caption: 
`*/ Github Stalker \\*
...
Zex.sendMessage(from,{image:krt.result,caption:mess.succes}, {quoted:m})
}
break
case 'join': {
if (!XeonTheCreator) return replygcxeon(mess.owner)
...
await Zex.groupAcceptInvite(result).then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
}
break
case 'toonce': case 'toviewonce': { 
if (!quoted) return replygcxeon(`Reply Image/Video`)
if (/image/.test(mime)) {
anuan = await Zex.downloadAndSaveMediaMessage(quoted)
Zex.sendMessage(m.chat, {image: {url:anuan}, caption: `Here you go!`, fileLength: "999", viewOnce : true},{quoted: m })
} else if (/video/.test(mime)) {
anuanuan = await Zex.downloadAndSaveMediaMessage(quoted)
Zex.sendMessage(m.chat, {video: {url:anuanuan}, caption: `Here you go!`, fileLength: "99999999", viewOnce : true},{quoted: m })
}
}
break
case 'fliptext': {
             ...
             break
                case 'listgc': {
                 ...
                 for (let i of anulistg) {
                     let metadata = await Zex.groupMetadata(i)
...
await Zex.sendMessage(i, { image:media,  caption: txt,mentions:participants.map(a => a.id) })
}
if(/video/.test(mime)){
let media = await quoted.download()
await Zex.sendMessage(i, { video:media,  caption: txt, mentions:participants.map(a => a.id) })
}
            }
...
case 'block': {
	...
	break
		...
		if (!XeonTheCreator) return replygcxeon(mess.owner)
		let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
		await Zex.updateBlockStatus(users, 'unblock').then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
	}
	break
...
pesan = q.split('|')[1]
let cekno = await Zex.onWhatsApp(num)
...
await Zex.sendMessage(num, {text:`You can also reply to the message by sending a message, if you don't want to reply, please type .leave and enter send button`}, { quoted : m })
...
break
case 'leave':{
if (m.isGroup && XeonTheCreator && command == "leave") return Zex.groupLeave(from)
...
if (other != "") Zex.sendMessage(other, {
text: "Bye..."
})
...
break
            case 'react': {
                ...
                Zex.sendMessage(m.chat, reactionMessage)
            }
            break
case 'ig':
case 'instagram':
const skrep = require('skrep');
if (!args || !args[0]) return m.reply('.instagram https://www.instagram.com/p/CK0tLXyAzEI');
if (!args[0].match(/(https:\/\/www.instagram.com)/gi)) return m.reply('Link Tidak Valid');

let dw = skrep.downloader.instagram.download(`${args[0]}`)
  .then(async (data) => {
    let media = data.media;
    let isVideo = typeof media === 'string' && (media.endsWith('.mp4') || media.includes('video'));
    let isImage = typeof media === 'string' && (media.endsWith('.jpg') || media.endsWith('.jpeg') || media.endsWith('.png'));

    if (isVideo) {
      // Jika tautan mengarah ke video
      Zex.sendMessage(m.chat, { video: { url: media } }, { quoted: m });
    } else if (isImage) {
      // Jika tautan mengarah ke foto
      Zex.sendMessage(m.chat, { caption: data.caption, image: { url: media } }, { quoted: m });
    } else {
      // Jika tautan tidak valid atau tidak dikenali
      return m.reply('Tautan tidak valid atau tidak dikenali.');
    }
  });
break
...
var groupe = await Zex.groupMetadata(from)
var members = groupe['participants']
var mems = []
members.map(async adm => {
mems.push(adm.id.replace('c.us', 's.whatsapp.net'))
})
Zex.sendMessage(from, {text: `\`\`\`「 ⚠️Warning⚠️ 」\`\`\`\n\nNo body is allowed to send virus in this group, member who send will be kicked immediately!`, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "off") {
...
await replygcxeon(`Please Type The Option\n\nExample: ${prefix + command} on\nExample: ${prefix + command} off\n\non to enable\noff to disable`)
  ...
  break
   case 'leavegc': {
                if (!XeonTheCreator) return replygcxeon(mess.owner)
                await Zex.groupLeave(m.chat).then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
            }
            break
case 'add': {
if (!m.isGroup) return replygcxeon(mess.group)
...
await Zex.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
}
break
case 'closetime': {
if (!m.isGroup) return replygcxeon(mess.group)
...
break
           case 'ephemeral': {
                ...
                if (args[0] === 'enable') {
                    await Zex.sendMessage(m.chat, { disappearingMessagesInChat: WA_DEFAULT_EPHEMERAL }).then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
                } else if (args[0] === 'disable') {
                    await Zex.sendMessage(m.chat, { disappearingMessagesInChat: false }).then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
                }
            }
            break
            case 'delete': case 'del': {
                ...
                if (!isBaileys) return replygcxeon('The message was not sent by a bot!')
                 Zex.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender } })
            }
            break
            case 'linkgroup': case 'linkgc': case 'gclink': case 'grouplink': {
                if (!m.isGroup) return replygcxeon(mess.group)
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                let response = await Zex.groupInviteCode(m.chat)
                Zex.sendText(m.chat, `https://chat.whatsapp.com/${response}\n\nGroup Link : ${groupMetadata.subject}`, m, { detectLink: true })
            }
            break
case 'opentime': {
if (!m.isGroup) return replygcxeon(mess.group)
...
if (!isBotAdmins) return replygcxeon(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Zex.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
}
break
    case 'setname': case 'setsubject': {
                ...
                await Zex.groupUpdateSubject(m.chat, text).then((res) => replygcxeon(mess.success)).catch((err) => replygcxeon(jsonformat(err)))
            }
            break
          case 'setdesc': case 'setdesk': {
                ...
                if (!isAdmins) return replygcxeon(mess.admin)
                if (!text) return replygcxeon('Text ?')
                await Zex.groupUpdateDescription(m.chat, text).then((res) => replygcxeon(mess.success)).catch((err) => replygcxeon(jsonformat(err)))
            }
            break
...
var mediz = await Zex.downloadAndSaveMediaMessage(quoted, 'ppgc.jpeg')
if (args[0] == `/panjang`) {
...
var memeg = await Zex.updateProfilePicture(m.chat, { url: mediz })
fs.unlinkSync(mediz)
replygcxeon(`Success`)
}
}
...
if (!isBotAdmins) return replygcxeon(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Zex.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
}
break
case 'demote': {
if (!m.isGroup) return replygcxeon(mess.group)
...
if (!isBotAdmins) return replygcxeon(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Zex.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => replygcxeon(jsonformat(res))).catch((err) => replygcxeon(jsonformat(err)))
}
break
case 'hidetag': {
if (!m.isGroup) return replygcxeon(mess.group)
if (!isAdmins && !XeonTheCreator) return replygcxeon(mess.admin)
if (!isBotAdmins) return replygcxeon(mess.botAdmin)
Zex.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: m })
}
break
case 'tagall': {
if (!m.isGroup) return replygcxeon(mess.group)
...
Zex.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: m })
}
break
case 'ebinary': {
...
Zex.sendMessage(m.chat, { caption: `Here you go!`, video: { url: data.watermark }}, {quoted:m})
})
}
break
case 'tiktokaudio':{
...
replygcxeon(mess.wait)
require('./lib/tiktok').Tiktok(q).then( data => {
Zex.sendMessage(m.chat, { audio: { url: data.audio }, mimetype: 'audio/mp4' }, { quoted: m })
})
}
break
case 'mediafire':
...
await Zex.sendMessage(from, {document:{url:medfr.link},jpegThumbnail : defaultpp, fileName:`Downloaded By ${m.pushName}.${medfr.mime}`, mimetype:`application/${mime}`},{quoted:m})
break
case 'google': {
...
case 'yts': case 'ytsearch': {
                ...
                Zex.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },  caption: teks }, { quoted: m })
            }
            break
case 'xxxxplay':{
if (!text) return replygcxeon(`Example : ${prefix+command} story wa anime`)
...
Zex.sendMessage(m.chat, { image : eek, caption: ngen }, { quoted: m})
}
break
...
if (!XeonTheCreator) return replygcxeon(mess.owner)
const getCase = (cases) => {
return "case"+`'${cases}'`+fs.readFileSync("zex.js").toString().split('case \''+cases+'\'')[1].split("break")[0]+"break"
}
...
let ceknya = await Zex.onWhatsApp(prrkek)
...
let ceknye = await Zex.onWhatsApp(bnnd)
...
teks += `\n*Total : ${prem.length}*`
Zex.sendMessage(m.chat, { text: teks.trim() }, 'extendedTextMessage', { quoted: m, contextInfo: { "mentionedJid": prem } })
break
case 'setexif': {
               if (!XeonTheCreator) return replygcxeon(mess.owner)
...
var memeg = await Zex.updateProfilePicture(botNumber, { url: medis })
fs.unlinkSync(medis)
replygcxeon(`Success`)
}
}
...
try {
let cret = await Zex.groupCreate(args.join(" "), [])
let response = await Zex.groupInviteCode(cret.id)
teks = `     「 Create Group 」
...
https://chat.whatsapp.com/${response}
       `
Zex.sendMessage(m.chat, { text:teks, mentions: await Zex.parseMention(teks)}, {quoted:m})
} catch {
replygcxeon("Error!")
}
}
...
let encmedia = await Zex.sendImageAsSticker(m.chat, await rmbg(media), m, { packname: global.packname, author: global.author })
...
case 'tomp4': case 'tovideo': {
            ...
            let { toAudio } = require('./lib/converter')
            let audio = await toAudio(media, 'mp4')
            Zex.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${Zex.user.name}.mp3`}, { quoted : m })
            }
            break
  ...
  await Zex.sendMessage(from, { image: medi, caption:"Here you go!"}, { quoted: m })
  ...
  case 'dare':
    ...
    "breakup with your best friend for 5hrs without telling him/her that its a dare",
     ...
     "send abusive words in any grup, excepting this grup, and send screenshot proof here"
              ...
              Zex.sendMessage(from, { image: bufferdare, caption: '_You choose DARE_\n'+ xeondare }, {quoted:m})
              break
                            break
       case 'truth':
              const truth =[
    ...
    "do you play pubg, if you then send ur id number"
              ...
              Zex.sendMessage(from, { image: buffertruth, caption: '_You choose TRUTH_\n'+ xeontruth }, {quoted:m})
              break
case 'checkme':
					neme = args.join(" ")
					bet = `${sender}`
...
Zex.sendMessage(from, { image: buff, caption: profile, mentions: [bet]},{quoted:m})
break
case 'toimg': {
...
Zex.sendMessage(m.chat, { image: buffer, jpegThumbnail: defaultpp }, { quoted: m })
fs.unlinkSync(ran)
})
}
break
...
Zex.downloadAndSaveMediaMessage(quoted, "gifee")
Zex.sendMessage(from, {sticker:fs.readFileSync("gifee.webp")},{quoted:m})
} else if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await Zex.sendImageAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return replygcxeon('Maximum 10 Seconds!')
let media = await quoted.download()
let encmedia = await Zex.sendVideoAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
await fs.unlinkSync(encmedia)
} else {
...
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await Zex.sendImageAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return replygcxeon('Send/Reply Images/Videos/Gifs With Captions ${prefix+command}\nVideo Duration 1-9 Seconds')
let media = await quoted.download()
let encmedia = await Zex.sendVideoAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
Zex.sendMessage(from, { text: `*${command}*\n\nName : ${q}\nAnswer : *${teng}%*` }, { quoted: m })
					break
case 'beautifulcheck':
				if (!text) return replygcxeon(`Tag Someone, Example : ${prefix + command} @Xeon`)
...
Zex.sendMessage(from, { text: `*${command}*\n\nNama : ${q}\nAnswer : *${tik}%*` }, { quoted: m })
					...
					Zex.sendMessage(from, { text: `Character Check : ${q}\nAnswer : *${taky}*` }, { quoted: m })
				     break
case 'awesomecheck':
  case 'greatcheck':
    case 'gaycheck':
...
if (mentionByReply) {
Zex.sendMessage(from, { text: 'Question : *' + cex + '*\nChecker : ' + `@${mentionByReply.split('@')[0]}` + '\nAnswer : ' + cek2 + '%', mentions: [mentionByReply] }, { quoted: m })
...
Zex.sendMessage(m.chat, { image: { url: anu }, caption: `${mess.success}` }, { quoted: m })
}
break
case 'glitchtext':
case 'writetext':
...
Zex.sendMessage(m.chat, { image: { url: haldwhd }, caption: `${mess.success}` }, { quoted: m })
}
break
case 'shadow': 
case 'write': 
...
Zex.sendMessage(m.chat, { image: { url: dehe }, caption: `${mess.success}` }, { quoted: m })
}
break
case 'tiktokgirl':
replygcxeon(mess.wait)
...
replygcxeon(mess.wait)						
 waifudd = await axios.get(`https://nekos.life/api/v2/img/${command}`)
            await Zex.sendMessage(m.chat,{ caption: mess.success, image: {url:waifudd.data.url} },{ quoted:m }).catch(err => {
                    return('Error!')
                })
break
case 'animewallpaper2': case 'animewall2': {
		...
		let { wallpaper } = require('./lib/scraperW')
                ...
                Zex.sendMessage(m.chat, { caption: `Title : ${result.title}\nCategory : ${result.type}\nDetail : ${result.source}\nMedia Url : ${result.image[2] || result.image[1] || result.image[0]}`, image: { url: result.image[0] } } , { quoted: m })
            }
            break
...
const i = Math.floor(Math.random() * wallpaper.length);     
            await Zex.sendMessage(m.chat, { caption: `*Query :* ${q}`, image: {url:wallpaper[i].image} }, { quoted: m} ).catch(err => {
                    return('Error!')
                })
...
if (/loli/.test(command)) heyy = await fetchJson('https://raw.githubusercontent.com/DGXeon/media/master/loli.json')
...
if (/neko/.test(command)) heyy = await fetchJson('https://raw.githubusercontent.com/DGXeon/media/master/neko.json')
if (/neko2/.test(command)) heyy = await fetchJson('https://raw.githubusercontent.com/DGXeon/media/master/neko2.json')
...
if (/waifu/.test(command)) heyy = await fetchJson('https://raw.githubusercontent.com/DGXeon/media/master/waifu.json')
...
Zex.sendMessage(m.chat, { image: { url: yeha }, caption : mess.success }, { quoted: m })
}
break
case '>':
case '=>':
     ...
     replygcxeon(`Done`)
      ...
      break
		...
		for (let res of anumojimix.results) {
		    let encmedia = await Zex.sendImageAsSticker(m.chat, res.url, m, { packname: global.packname, author: global.author, categories: res.tags })
		    await fs.unlinkSync(encmedia)
		}
	    }
	    break
...
Zex.sendMessage(m.chat, { video: { url: cejd.video_1 }, 
...
if (!AntiNsfw) return m.reply(mess.nsfw)
                ...
                Zex.sendMessage(m.chat, { video: { url: result912.video_1 }, caption: `${themeemoji} Title : ${result912.title}\n${themeemoji} Category : ${result912.category}\n${themeemoji} Mimetype : ${result912.type}\n${themeemoji} Views : ${result912.views_count}\n${themeemoji} Shares : ${result912.share_count}\n${themeemoji} Source : ${result912.link}\n${themeemoji} Media Url : ${result912.video_1}` }, { quoted: m })
            }
            break
case 'trap' :
if (!m.isGroup) return replygcxeon(mess.group)
...
Zex.sendMessage(m.chat, { caption: mess.success, image: { url: xeonyresult.url } }, { quoted: m })
break 
case 'animespank':
if (!m.isGroup) return m.reply(mess.group)
if (!AntiNsfw) return m.reply(mess.nsfw)
m.reply(mess.wait)						
 waifudd = await axios.get(`https://nekos.life/api/v2/img/spank`)     
            await Zex.sendMessage(m.chat, { caption:  `Here you go!`, image: {url:waifudd.data.url} },{ quoted:m }).catch(err => {
                    return('Error!')
                })
break
case 'ahegao':
...
replygcxeon(mess.wait)
  let assss = await axios.get ("https://api.waifu.pics/nsfw/blowjob")
    ...
    await Zex.sendMessage(m.chat,{video:bogif, gifPlayback:true },{quoted:m}).catch(err => {
    })
    break
case 'gifhentai':
if (!m.isGroup) return replygcxeon(mess.group)
...
var xeonyresultx = pickRandom(ahegaonsfw)
    await Zex.sendMessage(m.chat,{video:xeonyresultx, gifPlayback:true },{quoted:m}).catch(err => {
    })
    break
    case 'gifs': case 'foot': {
if (!m.isGroup) return replygcxeon(mess.group)
...
let mese = await Zex.sendMessage(m.chat, {image:{url:emoji.images[4].url}, caption: `Made by ${global.botname}`}, {quoted:m})
await Zex.sendMessage(from, {text:"reply #s to this image to make sticker"}, {quoted:mese})
})
}
break
case 'volume': {
if (!args.join(" ")) return replygcxeon(`Example: ${prefix + command} 10`)
media = await Zex.downloadAndSaveMediaMessage(quoted, "volume")
if (isQuotedAudio) {
rname = getRandom('.mp3')
...
Zex.sendMessage(from, {audio:jadie, mimetype: 'audio/mp4', ptt: true}, {quoted: m})
fs.unlinkSync(rname)
})
...
jadie = fs.readFileSync(rname)
Zex.sendMessage(from, {video:jadie, mimetype: 'video/mp4'}, {quoted: m})
fs.unlinkSync(rname)
})
} else {
...
let regex1 = /(?:https|git)(?::\/\/|@)github\.com[\/:]([^\/:]+)\/(.+)/i
    ...
    Zex.sendMessage(m.chat, { document: { url: url }, fileName: filename+'.zip', mimetype: 'application/zip' }, { quoted: m }).catch((err) => replygcxeon(mess.error))
break
case 'spotify':
if (!text) return replygcxeon(`Where is the link?`)
       ...
       const response = await Zex.sendMessage(m.chat, { image: { url: cover_url }, caption: details }, { quoted: m })
        const bufferpotify = await spotify.download()
        await Zex.sendMessage(m.chat, { audio: bufferpotify }, { quoted: response })
break
...
*${themeemoji} Example:* ${targetfine.data.list[0].example
    .replace(/\[/g, "")
    .replace(/\]/g, "")}`
   Zex.sendMessage(m.chat,{text:reply},{quoted:m})
} catch (err) {
    console.log(err)
...
Zex.sendMessage(m.chat, { text: xeonkak }, { quoted: m })
break
            case 'soulmate': {
...
{ quoted: m})        
            ...
            break
                ...
                Zex.sendMessage(m.chat, {caption: mess.success, image: { url: 'https://coffee.alexflipnote.dev/random' }}, { quoted: m })
            }
            break
            case 'wallpaper': {
		...
		let { wallpaper } = require('./lib/scraper')
                ...
                Zex.sendMessage(m.chat, {caption: `${themeemoji} Title : ${result.title}\n${themeemoji} Category : ${result.type}\n${themeemoji} Detail : ${result.source}\n${themeemoji} Media Url : ${result.image[2] || result.image[1] || result.image[0]}`, image: { url: result.image[0] }} , { quoted: m })
            }
            break
            case 'wikimedia': {
                if (!text) return replygcxeon('Enter Query Title')
...
let teks = `${text}`
{
Zex.relayMessage(from, { requestPaymentMessage: { Message: { extendedTextMessage: { text: `${xeonorwot}`, currencyCodeIso4217: 'INR', requestFrom: '0@s.whatsapp.net', expiryTimestamp: 8000, amount: 1, contextInfo:{"externalAdReply": {"title": `PAPA XEON`,"body": ` ${xeonytimewisher} my friend ${pushname}`,
...
Zex.relayMessage(m.chat, { reactionMessage }, { messageId: '🦄' })
}
break
...
let e = String(err)
Zex.sendMessage(owner + "@s.whatsapp.net", { text: "🚩 Eror, Please Report Author. " + util.format(e), 
contextInfo:{
forwardingScore: 9999999, 
isForwarded: true
}})
